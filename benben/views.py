    ...
    attachments_info = [
        {
            ...
            "markdown": f"![{asset.asset_id}]({preferred_url})" if preferred_url else None,
        }
    ]
    snippet = ""
    message = ""
    ...
    if ext_lower == ".pdf":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for generated in converted_files:
                ...
                preferred_image_url = image_oss_url or image_url
                generated_infos.append(
                    {
                        "name": image_asset.name,
                        "assetId": image_asset.asset_id,
                        "localUrl": image_url,
                        "ossUrl": image_oss_url,
                        "preferredUrl": preferred_image_url,
                        "markdown": f"![{image_asset.asset_id}]({preferred_image_url})" if preferred_image_url else None,
                    }
                )
            if generated_infos:
                attachments_info.extend(generated_infos)
                snippet_lines = [
                    f"![{info.get('assetId') or info.get('name', '')}]({info.get('preferredUrl')})"
                ...
                if preferred_url:
                    snippet = f"[{asset.asset_id or filename}]({preferred_url})\n"
                if error:
        ...
        if preferred_url:
            label = asset.asset_id or filename
            if ext_lower in _IMAGE_EXTENSIONS:
                snippet = f"![{label}]({preferred_url})\n"
            else:
                snippet = f"[{label}]({preferred_url})\n"
        message = "附件已上传并插入引用"
